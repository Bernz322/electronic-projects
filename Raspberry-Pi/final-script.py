...
# Capturing of images for face recognition
...
# Training captured images/ models which is the basis of face recognition
def train_model():
    ...
    # initialize the list of known encodings and known names (for debugging purposes)
    knownEncodings = []
    knownNames = []
    ...
    while True:
        ...
        for encoding in encodings:
            ...
            # update the list of names
                ...
                # If not recognized, print name the person Unknown
                defaultName = "Unknown"
...
# RC522 RFID Scanning and the creation of attendance to the DB
def rfid_scan():
    with canvas(device) as draw:
    ...
    try:
        ...
        time.sleep(1)
        if faceName.strip() == tagName.strip(): # If face recognition name is the same with the embedded RFID tag name then record attendance
            # Send get req to the API to check if the RFID tag exists
            x = requests.get('http://your-api-route.com/api/user/' + str(id))
            if x.status_code == 404:
                # If RFID tag does not exist in the DB (404), display message to the 1.3in OLED
                print("Tag not found")
                with canvas(device) as draw:
                ...
                # If RFID is recognized then fetch phone number and their name
                phone = (x.json()['phone'])
                name = (x.json()['name'])
                # Welcome the student through the OLED
                with canvas(device) as draw:
                    draw.text((10, 10),  "Welcome "+str(name),  font=font_mcsmall, fill=255)
                    draw.text((10, 30),  "Please wait",  font=font_mcsmall, fill=255)
                phone = '+63'+phone
                # SIM900A GSM Module only accepts bytes so we have to convert the retrieved phone number as it is in string format
                phone = bytes(phone, 'utf-8')
                # Send POST Req to the API to create the attendance of that student on the same day it was scanned
                y = requests.post(
                    'http://your-api-route.com/api/attendance/', data={"rfid": id})
                with canvas(device) as draw:
                    draw.text((10, 30),  "Attendance recorded!",  font=font_mcsmall, fill=255)
                time.sleep(1)
                sendMessage(phone)
        else: # If face recognition name is not the same to the embedded RFID tag student name
            with canvas(device) as draw:
...
tagName = ""    # Embedded RFID tag name variable
...
while True:
    ...
    if option == "y" or option == "Y":  # For student registration
    ...
    elif option == "n" or option == "N":    # To start face recognition and attendance recording
        # Thread both functions as both of them must run simultaneously
